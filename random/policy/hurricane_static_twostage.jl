...
#solve the model.
start=time();
LB_st2SSP, UB_st2SSP, iter_st2SSP, xval_st2SSP, fval_st2SSP, θval_st2SSP = RH_2SSP_solve_roll(s,t_roll,nbstages1,nbstages2,master,subproblem,x,f,θ,y1,z1,v1,ConsFB,dCons,rCons,feas_m,feas_y,feas_z,feas_v,feas_ConsFB,feas_dCons,feas_rCons);
elapsed = time() - start;

#eval the model.
start=time();
objs_st2SSP, st2SSP_bar, st2SSP_low, st2SSP_high = static_2SSP_evaluation();
elapsed2 = time() - start;
