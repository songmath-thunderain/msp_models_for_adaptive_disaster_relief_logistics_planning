	...
	if τ !== nothing
		...
		for t_roll=2:(T-1)
			...
			if t_roll == (T-1)
				...
				set_normalized_rhs(rCons,
								-sum(sum(cb[i,ii,t_roll]*fval_Roll[i,ii,2] for ii=1:Ni) for i=1:N0)
								+sum(ch[i,t_roll]*xval_Roll[i,2] for i=1:Ni)  
								+sum(fval_Roll[N0,i,2] for i=1:Ni)*h[t_roll]
								);
				# Also need to update the coefficients of y[i,j] variables in the 2nd stage
				for i=1:Ni
					for j=1:Nj
						set_objective_coefficient(subproblem, y2[i,j], ca[i,j,t_roll]);
					end
				end
				optimize!(subproblem); 
      			objs_RH2SSP[s,t_roll] = objective_value(subproblem);
