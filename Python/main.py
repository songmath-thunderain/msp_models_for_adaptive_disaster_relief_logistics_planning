...
if __name__ == "__main__":
    ...
    parser.add_argument("-fc", "--flow_capacity", type = float, choices = [0.25,0.5,1,2,4], required = False, help = "flow capacity level: needs to be 0.25, 0.5, 1, 2, or 4")
    ...
    if instance_option == -1:
        ...
        networkInstance.input_from_Case_new(cost_structure,safe_time,tau,netFolderPath,hurricaneInstance,arc_option,0,fc_level,penalty_level);
        ...
        netFolderPath = 'data/case-study/SC-network/';
        networkInstance.input_from_Case_new(cost_structure,safe_time,tau,netFolderPath,hurricaneInstance,arc_option,1,fc_level,penalty_level);
        ...
        if option == 4 or option == 5:
            ...
            if write_option == 1:
                with open(outputpath+'FC_SA_WSresults.csv', 'a') as myfile:
                    writer = csv.writer(myfile, delimiter =',')
                    writer.writerow([instance_option,args.flow_capacity,safe_time,obj,CI,train_time,test_time])
                with open(outputpath+'FC_WSresults-KPI.csv', 'a') as myfile:
                    writer = csv.writer(myfile, delimiter =',')
                    writer.writerow([instance_option,args.flow_capacity,safe_time]+KPIvec)
        ...
        if option == 4 or option == 5:
            ...
            if write_option == 1:
                with open(outputpath+'Penalty_SA_WSresults.csv', 'a') as myfile:
                    writer = csv.writer(myfile, delimiter =',')
                    writer.writerow([instance_option,args.penalty,safe_time,obj,CI,train_time,test_time])
                with open(outputpath+'Penalty_WSresults-KPI.csv', 'a') as myfile:
                    writer = csv.writer(myfile, delimiter =',')
                    writer.writerow([instance_option,args.penalty,safe_time]+KPIvec)
    else:
        if option == -1 or option == 5:
