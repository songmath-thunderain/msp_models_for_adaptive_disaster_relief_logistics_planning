...
class FA:
    def __init__(self,inputParams,solveParams,hurricaneData,networkData,option):
    ...
    def FOSDDP_backward_pass_oneSP_iteration(self, xval, thetaval, in_sample):
        ...
        for t in range(len(in_sample)-1, 0, -1):
            ...
            if self.option == 0:
                # Only generate cuts to a single node
                Qvalue = 0;
                for k in range(len(nodeLists[t])):
                    if P_joint[sample_n][nodeLists[t][k]] > self.hurricaneData.smallestTransProb:
                        Qvalue += Q[nodeLists[t][k]] * P_joint[sample_n][nodeLists[t][k]]              					# check if cut is violated at the sample path encountered in the forward pass
                
                # check if cut is violated at the sample path encountered in the forward pass
                if (Qvalue - thetaval[t - 1]) / max(1e-10, abs(thetaval[t - 1])) > self.solveParams.cutviol and abs(Qvalue - thetaval[t - 1]) > self.solveParams.cutviol:
                    cutviolFlag = True;
                    cutcoef = [0] * Ni
                    cutrhs_xval = 0
                    for k in range(len(nodeLists[t])):
                        if P_joint[sample_n][nodeLists[t][k]] > self.hurricaneData.smallestTransProb:
                            for i in range(Ni):
                                ...
                                cutcoef[i] += tempval
                                cutrhs_xval += tempval * xval[i][t - 1]
                    self.m[t - 1, sample_n].addConstr(
                        self.theta[t - 1, sample_n] - gp.quicksum(
